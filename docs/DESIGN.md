...
Базовый контракт из основной ветки сохраняется: панели имеют тонкую границу,
скругление и мягкую тень; таблицы поддерживают горизонтальную прокрутку;
loading/error/empty, focus и disabled состояния должны оставаться видимыми.
До 1050 px многоколоночные секции упрощаются, а до 720 px навигация и карточки
переходят в компактную мобильную раскладку.
