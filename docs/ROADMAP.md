...
- Этап 6 — доменный CPU-бэктест реализован в работах, исторически называвшихся
  ...
  долгом по P&L, указанным в selected record `prompts/STAGES.md`; `TD-UI-001`
  закрыт production browser acceptance на 1x–100x и 20 000 свечей.
...
- `TD-UI-001` — `VERIFIED/CLOSED`: один replay data-reveal clock подтверждён
  component regression tests и production browser acceptance на всех
  поддерживаемых скоростях 1x–100x и границе 20 000 свечей.
...
Подробные критерии и текущий NEXT находятся в
