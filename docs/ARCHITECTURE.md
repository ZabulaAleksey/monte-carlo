...
## Границы развития по дорожной карте

Текущая реализованная архитектура выше остаётся источником истины для работающего
кода. Порядок будущего развития задаёт
[`MONTE_CARLO_ROADMAP_13_TO_28.md`](MONTE_CARLO_ROADMAP_13_TO_28.md): этапы 1–13
образуют базовую последовательность, а этапы 14–28 являются опциональным
расширением data-platform и не запускаются без отдельного решения.

Будущие источники данных, хранилища и вычислители подключаются через доменные
порты и адаптеры. Broker API, MetaApi и MT5 fallback не должны менять доменную
модель, формат provenance или эталонную CPU-реализацию. CPU остаётся источником
корректности для ускоренных backend-реализаций.

Проекту не требуется отдельный MCP-сервер для этой дорожной карты. MT5, Broker
